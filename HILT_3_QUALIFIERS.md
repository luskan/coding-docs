...
The key here is `(no qualifier, WordsRepository)`: the requested type together with the absence of
a qualifier. Both `@Binds` methods offer an answer for that exact same key, so Hilt refuses at
compile time rather than pick one.
...
- These annotations need no properties. A qualifier's identity is its fully qualified annotation
  type -- package plus class name -- and the values of any properties it declares. Therefore,
  `@BasicWords WordsRepository` and `@FancyWords WordsRepository` are different keys. Likewise,
  `@Named("basic")` and `@Named("fancy")` are different keys because their `value` properties differ.
